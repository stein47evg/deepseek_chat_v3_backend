...
from app.utils.file_utils import scan_directory_for_disk, is_allowed_file
...
class FileManagerService:
    ...
    @staticmethod
    def get_disk_files(db: Session, project: Project, show_ignored: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        for file in disk_files:
            ...
            result.append({
                **file,
                ...
                "isAllowed": is_allowed_file(normalized_path)
            })
        
        return result

    @staticmethod
    def soft_delete_file(db: Session, project: Project, filename: str) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def hard_delete_file(db: Session, project: Project, filename: str) -> Dict[str, Any]:
        """
    ...
    def sync_disk_to_db(db: Session, project: Project) -> Dict[str, Any]:
        ...
        for file_info in disk_files:
            ...
            if existing:
                ...
                try:
                    with open(os.path.join(project.folder_path, filename), 'r', encoding='utf-8') as f:
                        content = f.read()
    ...
    def flatten_history(db: Session, project: Project) -> Dict[str, Any]:
        ...
        current = db.query(Snapshot).filter(
        ...
        deleted_versions = db.query(FileVersion).filter(
            FileVersion.chat_id == chat.id,
            FileVersion.is_current == False
        ).delete()
        
        # 5. Удаляем все старые снимки
        deleted_snapshots = db.query(Snapshot).filter(
            Snapshot.project_id == project.id
        ).delete()
