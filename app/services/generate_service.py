...
class GenerateService:
    ...
    def _get_system_prompt(self, request: GenerateRequest) -> str:
        ...
        default = self.db.query(SystemPrompt).filter(
    ...
    def _get_history_start_files(self) -> List[FileVersion]:
        ...
        first_with_files = self.db.query(Message).filter(
    ...
    def _get_history_messages(self, strategy: str) -> List[Message]:
        ...
        if strategy == "flexible":
            ...
            last_with_files = self.db.query(Message).filter(
                Message.chat_id == self.chat_id,
                Message.role == "user",
                Message.context_data.isnot(None),
                func.json_length(Message.context_data, '$.file_version_ids') > 0
    ...
    async def generate_stream(
        ...
        try:
            ...
            if finish_reason == "length":
                ...
                logger.warning(f"Достигнут лимит токенов ({max_tokens}). Ответ обрезан.")
    ...
    async def continue_stream(
        ...
        try:
            ...
            async for chunk_data in self.client.generate(
                ...
                temperature=continue_request.temperature or settings.DEFAULT_TEMPERATURE,
                max_tokens=max_tokens,
                stream=use_stream
            ):
                if isinstance(chunk_data, dict):
            ...
            if finish_reason == "length":
                ...
                assistant_message.cost = calculate_cost(input_tokens, output_tokens, continue_request.model or "flash")
                self.db.commit()
    ...
    def _log_request_context(self, request: GenerateRequest, context: Dict) -> None:
        ...
        logger.info(f"Приложено файлов: {len(request.selected_files) if request.selected_files else 0}")
        logger.info("=" * 80)
